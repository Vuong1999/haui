...
class MyAdapter extends ArrayAdapter {
    ...
    int layoutID;
    ArrayList<Nguoi> list=null;

    public MyAdapter(Activity context , int layoutID, ArrayList<Nguoi> objects) {
    ...
    public View getView (int position, View convertView, ViewGroup parent){
        ...
        if((list.size()>0)&&(position>=0)){
            ...
            txtStt.setText(list.get(position).getTen()+"");
            txtNoiDung.setText(list.get(position).getTuoi()+"");
        }
        return convertView;
    }
}
